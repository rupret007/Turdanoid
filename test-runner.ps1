...
if ($game -match 'window\.gameInstance = gameInstance;') { Pass("legacy breakout exports game instance to window") } else { Fail("legacy breakout exports game instance to window") }
...
if ($game -match 'document\.addEventListener\(''visibilitychange'', this\.boundHandlers\.visibilityChange\);' -and $game -match 'document\.removeEventListener\(''visibilitychange'', this\.boundHandlers\.visibilityChange\);') { Pass("legacy breakout visibility handler wired") } else { Fail("legacy breakout visibility handler wired") }
if ($game -match '(?s)handleWindowBlur\(\)\s*\{\s*this\.controls\.left = false;\s*this\.controls\.right = false;\s*this\.controls\.pointerActive = false;\s*if \(this\.gameRunning\) this\.togglePause\(true\);' -and $game -match 'handleVisibilityChange\(\)\s*\{\s*if \(document\.hidden\) \{\s*this\.handleWindowBlur\(\);') { Pass("legacy breakout blur clears input and handles hidden tabs") } else { Fail("legacy breakout blur clears input and handles hidden tabs") }
if ($game -match 'ensureGameLoopRunning\(\)' -and $game -match 'stopGameLoop\(\)' -and $game -match 'if \(this\.paused\) \{\s*this\.stopGameLoop\(\);' -and $game -match 'if \(this\.gameRunning && !this\.paused\) this\.ensureGameLoopRunning\(\);') { Pass("legacy breakout pauses and resumes render loop intentionally") } else { Fail("legacy breakout pauses and resumes render loop intentionally") }
...
if ($tris -match 'function stopLoop\(\)' -and $tris -match 'function ensureLoopRunning\(\)' -and $tris -match 'if \(paused\) stopLoop\(\);\s*else ensureLoopRunning\(\);' -and $tris -match 'stopLoop\(\);\s*if \(!paused && !gameOver && !onboardingOpen\) \{') { Pass("turdtris stops background loop work while paused") } else { Fail("turdtris stops background loop work while paused") }
...
if ($turd -match 'function flushDestroyPoints\(' -and $turd -match 'const earned = flushDestroyPoints\(\);' -and $turd -match 'maybeDrop\(b\.x\+b\.w/2, b\.y\+b\.h/2\)') { Pass("flush destroy scoring") } else { Fail("flush destroy scoring") }
