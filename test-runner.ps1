...
if ($game -match '(?s)handleWindowBlur\(\)\s*\{\s*this\.controls\.left = false;\s*this\.controls\.right = false;\s*this\.controls\.pointerActive = false;\s*if \(this\.gameRunning && !this\.paused\) this\.togglePause\(true\);' -and $game -match 'handleVisibilityChange\(\)\s*\{\s*if \(document\.hidden\) this\.handleWindowBlur\(\);') { Pass("legacy breakout blur clears input and handles hidden tabs") } else { Fail("legacy breakout blur clears input and handles hidden tabs") }
